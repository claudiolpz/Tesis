...
LinearizerAbsTaylor::LinearizerAbsTaylor(const System& _sys, point_policy point):
		Linearizer(_sys.nb_var), sys(_sys),
			m(sys.f_ctrs.image_dim()), goal_ctr(-1),
...
int LinearizerAbsTaylor::linear_restrict(const IntervalVector& box) {
    ...
    // Determine expansion point based on policy
    if (point == MID) {
		exp_point = box.mid();
    }
    ...
    else if (point == SA_PSO) {
		try {
            ibex::SA_PSO sa_pso(box, sys, 70, 10, 115.0, 0.89, 0.0044); // box, sys, particulas, iteraciones maximas, temp_inicial, cooling, temp_min
            sa_pso.optimize(); 
            exp_point = sa_pso.gbest_position.mid();
        } catch (const std::exception& e) {
            throw ibex::Exception(("Error al inicializar SA_PSO: " + std::string(e.what())).c_str());
        }
	}
